...
2. If you have already [installed R, RStudio, and Quarto](../reading/week-1.1.qmd), you can download the [practice activity template](../student-versions/practice-activities/pa1.qmd) here.
...
*Hint:* This is what is called a "relational" comparison, where you compare an object to a number and R will give you a vector of `TRUE`s and `FALSE`s based on whether the comparison is / is not met. You can then use these `TRUE`s and `FALSE`s as numbers, since `TRUE` = `1` and `FALSE` = `0` in R land.
